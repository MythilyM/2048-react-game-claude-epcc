...
describe("slideRow", () => {
  it("slides tiles to the left", () => {
    expect(slideRow([0, 2, 0, 4], 1).row).toEqual([2, 4, 0, 0]);
  });

  it("merges equal adjacent tiles", () => {
    const result = slideRow([2, 2, 0, 0], 1);
    expect(result.row).toEqual([4, 0, 0, 0]);
    expect(result.score).toBe(4);
  });

  it("does not double-merge in one pass", () => {
    const result = slideRow([2, 2, 2, 2], 1);
    expect(result.row).toEqual([4, 4, 0, 0]);
    expect(result.score).toBe(8);
  });

  it("merges highest pair first (left side)", () => {
    const result = slideRow([2, 2, 4, 0], 1);
    expect(result.row).toEqual([4, 4, 0, 0]);
    expect(result.score).toBe(4);
  });

  it("returns zero score when no merges", () => {
    expect(slideRow([2, 4, 8, 16], 1).score).toBe(0);
  });

  it("handles all zeros", () => {
    expect(slideRow([0, 0, 0, 0], 1).row).toEqual([0, 0, 0, 0]);
  });
});
